...
/// <summary>
/// Test-only helper that lives under the framework namespace so it is skipped by 's frame-skip logic, so
/// tests can verify the outer-caller frame is returned.
/// </summary>
internal static class TestStackWalkerHelper
{
    ...
    /// <summary>
    /// Captures a stack from inside the framework namespace and runs it through so tests can verify that this
    ...
    /// <summary>
    /// Returns a stack trace captured from inside the framework namespace so callers can feed it to  and verify
    /// framework frames are stripped.
    /// </summary>
