...
overwrites the oldest, so an early error can drop out of the window. Read `DroppedCount` to find
out whether that happened. A full ring reads the same as a wrapped one without it.
